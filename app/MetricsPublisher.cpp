...
MetricsPublisher::MetricsPublisher(
    ...
    Tasks::TaskManager* taskMgr,
    Queues::InQueuesContainer* inQueues,
...
void MetricsPublisher::onTimer(boost::system::error_code ec) {
    ...
    collectAndBroadcast();
    timer_.expires_after(interval_);
    timer_.async_wait([self = shared_from_this()](boost::system::error_code ec2) {
        self->onTimer(ec2);
    });
}
...
void MetricsPublisher::collectAndBroadcast() {
    ...
    // Order count
    size_t orderCount = 0;
    orderStorage_->forEachOrder([&orderCount](const IdT&, const OrderEntry&) { ++orderCount; });
