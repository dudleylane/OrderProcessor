...
template <typename T> class InterLockCache
{
public:
    ...
    T *popFront()
    {
        do
        {
            ...
            if (cur != null)
            {
                ...
                Node *expected = cur;
                nextFree_.compare_exchange_strong(expected, cur->next_, std::memory_order_release,
    ...
    void pushBack(T *val)
    {
        ...
        while (cur->next_ != free)
        {
            ...
            if (cur->val_.compare_exchange_weak(expected, val, std::memory_order_release, std::memory_order_relaxed))
            {
                ...
                nextNull_.compare_exchange_strong(cur, cur->next_, std::memory_order_release,
                                                  std::memory_order_relaxed);
                return;
            }
...
private:
    struct Node
    {
        ...
        Node(Node *next, T *val) : next_(next)
        {
            val_.store(val);
        }
...
template <typename T> InterLockCache<T> *InterLockCache<T>::instance_ = nullptr;
...
template <typename V> class CacheAutoPtr
{
public:
