...
public:
    ...
    static void *allocateOnNode(size_t size, int node) noexcept
    {
        ...
        int ret = mbind(ptr, alignedSize, MPOL_BIND, &nodemask, maxnode, MPOL_MF_MOVE);
    ...
    static void *allocateLocal(size_t size) noexcept
    {
        ...
        int ret = mbind(ptr, alignedSize, MPOL_PREFERRED, nullptr, 0, 0);
 ...
 */
template <typename T> class NumaNodeAllocator
{
public:
    using value_type = T;
    ...
    template <typename U> NumaNodeAllocator(const NumaNodeAllocator<U> &other) noexcept : node_(other.node()) {}
    ...
    template <typename U> bool operator==(const NumaNodeAllocator<U> &other) const noexcept
    {
