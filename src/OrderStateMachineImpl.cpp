...
{
    return (stateZone1Id_ == val.stateZone1Id_) && (stateZone2Id_ == val.stateZone2Id_);
}
...
void OrdStateImpl::processReceive(OrderEntry **orderData, OrdState::onRplOrderReceived const &evnt)
{
    ...
    *orderData = evnt.orderStorage_->save(**orderData, evnt.generator_);
    ...
    std::unique_ptr<Operation> op(new EnqueueOrderEventTrOperation<onReplaceReceived>(
        *origOrder, onReplaceReceived((*orderData)->orderId_), (*orderData)->instrument_.getId()));
    evnt.transaction_->addOperation(op);
}
...
void OrdStateImpl::processReject(OrderEntry *orderData, OrdState::onRplOrderRejected const &evnt)
{
    ...
    std::unique_ptr<Operation> op(
        new EnqueueOrderEventTrOperation<onReplaceRejected>(*origOrder, onReplaceRejected(orderData->orderId_)));
    evnt.transaction_->addOperation(op);
}
