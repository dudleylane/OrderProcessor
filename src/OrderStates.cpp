...
template <class FSM> void Rejected::on_entry(onReplace const &evnt, FSM &)
{
    ...
    std::unique_ptr<Operation> op(
        new CreateRejectExecReportTrOperation("", COP::REJECTED_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void Rejected::on_entry(onRecvOrderRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateRejectExecReportTrOperation(evnt.reason_, COP::REJECTED_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void Rejected::on_entry(onRecvRplOrderRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Rejected::on_entry(onExternalOrderRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Rejected::on_entry(onOrderRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Rejected::on_entry(onRplOrderRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Rejected::on_entry(onExternalOrder const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void New::on_entry(onExternalOrder const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op1(
        new CreateExecReportTrOperation(COP::NEW_ORDSTATUS, COP::NEW_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op1);
}

template <class FSM> void New::on_entry(onOrderReceived const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void New::on_entry(onReplace const &evnt, FSM &)
{
    ...
    std::unique_ptr<Operation> op(
        new CreateReplaceExecReportTrOperation(evnt.origOrderId_, COP::NEW_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void New::on_entry(onNewDay const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void New::on_entry(onContinue const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Expired::on_entry(onExpired const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateExecReportTrOperation(COP::EXPIRED_ORDSTATUS, COP::EXPIRED_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void Expired::on_entry(onRplOrderExpired const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void CnclReplaced::on_entry(onExecCancel const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void CnclReplaced::on_entry(onInternalCancel const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void CnclReplaced::on_entry(onExecReplace const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Suspended::on_entry(onSuspended const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateExecReportTrOperation(COP::SUSPENDED_ORDSTATUS, COP::SUSPENDED_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void DoneForDay::on_entry(onFinished const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateExecReportTrOperation(COP::DFD_ORDSTATUS, COP::DFD_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void New::on_entry(onTradeCrctCncl const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateCorrectExecReportTrOperation(evnt.correct(), NEW_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void PartFill::on_entry(onTradeExecution const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateTradeExecReportTrOperation(evnt.trade(), PARTFILL_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void PartFill::on_entry(onTradeCrctCncl const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateCorrectExecReportTrOperation(evnt.correct(), PARTFILL_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void PartFill::on_entry(onNewDay const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateExecReportTrOperation(PARTFILL_ORDSTATUS, COP::RESTATED_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void PartFill::on_entry(onContinue const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Filled::on_entry(onTradeExecution const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateTradeExecReportTrOperation(evnt.trade(), FILLED_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void Suspended::on_entry(onTradeCrctCncl const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateCorrectExecReportTrOperation(evnt.correct(), SUSPENDED_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void CnclReplaced::on_entry(onTradeCrctCncl const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void Expired::on_entry(onTradeCrctCncl const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateCorrectExecReportTrOperation(evnt.correct(), EXPIRED_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void DoneForDay::on_entry(onTradeCrctCncl const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(
        new CreateCorrectExecReportTrOperation(evnt.correct(), DFD_ORDSTATUS, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void GoingCancel::on_entry(onCancelReceived const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    }
    std::unique_ptr<Operation> op(new CreateExecReportTrOperation(
        evnt.order4StateMachine_->status_, COP::PEND_CANCEL_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void GoingReplace::on_entry(onReplaceReceived const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
    ...
    std::unique_ptr<Operation> op(new CreateExecReportTrOperation(
        evnt.order4StateMachine_->status_, COP::PEND_REPLACE_EXECTYPE, *evnt.order4StateMachine_));
    evnt.transaction_->addOperation(op);
}

template <class FSM> void NoCnlReplace::on_entry(onCancelRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
...
template <class FSM> void NoCnlReplace::on_entry(onReplaceRejected const &evnt, FSM &)
{
    if (evnt.testStateMachine_)
    {
        return;
