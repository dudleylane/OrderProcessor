...
void SubscrManager::addSubscription(const std::string &name, OrderFilter *filter, const SubscriberIdT &handlerId)
{
    ...
    bool isGeneral = !filter->getInstrument(&instr);
    {
        ...
        if (subscriptionsBySubscriber_.end() == sit)
        {
            ...
            sit = subscriptionsBySubscriber_.insert(SubscriptionsBySubscriberT::value_type(handlerId, tmp)).first;
        }
        if (isGeneral)
        {
        ...
        else
        {
            ...
            {
                git = sit->second[ORDER_SUBSCRIPTION]
                          .groupedByInstruments_
                          .insert(SubscriptionsByInstrumentGroupT::value_type(instr, SubscriptionsT()))
                          .first;
            }
...
{
    {
             ...
             hit != subscriptionsBySubscriber_.end(); ++hit)
        {
            ...
            {
                SubscriptionsT::const_iterator sit = git->second.begin(), eit = git->second.end();
