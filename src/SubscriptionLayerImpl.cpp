...
void SubscriptionLayerImpl::process(const OrderEntry &order, const MatchedSubscribersT &subscribers)
{
    ...
    for (const auto &subscriberId : subscribers)
    {
        ...
        aux::ExchLogger::instance()->note(std::string("SubscriptionLayer::process() - notifying subscriber ") +
                                          std::to_string(subscriberId.id_) + " about order " +
                                          std::to_string(order.orderId_.id_));
    }

    aux::ExchLogger::instance()->note(std::string("SubscriptionLayer::process() - processed ") +
                                      std::to_string(subscribers.size()) + " subscribers for order " +
                                      std::to_string(order.orderId_.id_));
}
