...
TEST(FixFullPipelineTest, EveryComponentTouched)
{
    ...
    ProcessorParams params(IdTGenerator::instance(), OrderStorage::instance(), orderBook.get(), inQueues.get(),
    ...
    // [FixGateway]
    auto gateway =
        std::make_unique<FixGateway>(inQueues.get(), WideDataStorage::instance(), OrderStorage::instance(), clearingId);
    ...
    EXPECT_EQ("GBPUSD", sellOrder->instrument_.get().symbol_) << "[WideDataStorage] instrument resolved";
    EXPECT_EQ("PIPELINE_ACCT", sellOrder->account_.get().account_) << "[WideDataStorage] account resolved";
    EXPECT_EQ("PIPELINE_CLR", sellOrder->clearing_.get().firm_) << "[WideDataStorage] clearing resolved";
    ...
    EXPECT_NE(sellOrder->orderId_.id_, buyOrder->orderId_.id_) << "[IdTGenerator] unique IDs for distinct orders";

    // --- [StateMachine] Full lifecycle: Rcvd_New → New → Filled ---
    EXPECT_EQ(FILLED_ORDSTATUS, sellOrder->status_) << "[StateMachine] sell reached FILLED";
    EXPECT_EQ(FILLED_ORDSTATUS, buyOrder->status_) << "[StateMachine] buy reached FILLED";

    // --- [OrderMatcher + DeferedEvents] Trade executed — qty decremented ---
    EXPECT_EQ(500u, sellOrder->cumQty_) << "[OrderMatcher] sell cumQty shows fill";
    EXPECT_EQ(0u, sellOrder->leavesQty_) << "[OrderMatcher] sell leavesQty shows complete fill";
    EXPECT_EQ(500u, buyOrder->cumQty_) << "[OrderMatcher] buy cumQty shows fill";
    EXPECT_EQ(0u, buyOrder->leavesQty_) << "[OrderMatcher] buy leavesQty shows complete fill";
    ...
    EXPECT_EQ(sellOrder, sellById) << "[OrderStorage] locateByOrderId works";
    OrderEntry *sellByClId = OrderStorage::instance()->locateByClOrderId(sellKey);
    EXPECT_EQ(sellOrder, sellByClId) << "[OrderStorage] locateByClOrderId works";

    // --- [TrOperations] Execution entries created ---
    EXPECT_GT(sellOrder->executions_.get()->size(), 0u) << "[TrOperations] sell has execution entries";
    EXPECT_GT(buyOrder->executions_.get()->size(), 0u) << "[TrOperations] buy has execution entries";
    ...
    for (const auto &ev : *sellOrder->executions_.get())
    {
        ...
        {
            foundTradeExec = true;
            EXPECT_TRUE(ex->execId_.isValid()) << "[TrOperations] trade execution has valid execId";
            break;
        }
    }
    EXPECT_TRUE(foundTradeExec) << "[TrOperations] sell order has TRADE execution in storage";
    ...
    EXPECT_FALSE(topBid.isValid()) << "[OrderBookImpl] no bids remaining after full fill";
    EXPECT_FALSE(topAsk.isValid()) << "[OrderBookImpl] no asks remaining after full fill";

    // --- [TransactionScopePool] Arena allocation used (zero cache misses) ---
    const TransactionScopePool *pool = evtProc->scopePool();
    ASSERT_NE(nullptr, pool);
    EXPECT_EQ(0u, pool->cacheMisses()) << "[TransactionScopePool] zero heap fallback — arena allocation worked";

    // --- [MultiOutQueues] Fan-out verified — both delegates received events ---
    EXPECT_GT(recordingOut->reportCount(), 0u) << "[MultiOutQueues] recording delegate received reports";
    EXPECT_GT(countingOut->count_.load(), 0) << "[MultiOutQueues] counting delegate received reports";
    ...
    for (const auto &r : reports)
    {
        ...
        {
            tradeReports++;
            EXPECT_TRUE(r.execId.isValid()) << "[OutQueues] trade report has valid execId";
            EXPECT_TRUE(r.orderId.isValid()) << "[OutQueues] trade report has valid orderId";
        }
    }
    EXPECT_GE(tradeReports, 2) << "[OutQueues] at least 2 trade reports (one per side)";
    ...
    // --- [OrderCodec] Round-trip encode/decode preserves filled order ---
    {
        ...
        EXPECT_EQ(sellOrder->orderId_, codecId) << "[OrderCodec] encoded orderId matches";

        OrderEntry *decoded = codec.decode(codecId, version, buf.data(), buf.size());
        ASSERT_NE(nullptr, decoded) << "[OrderCodec] decode succeeded";

        EXPECT_EQ(FILLED_ORDSTATUS, decoded->status_) << "[OrderCodec] round-trip preserves FILLED status";
        EXPECT_EQ(500u, decoded->cumQty_) << "[OrderCodec] round-trip preserves cumQty";
        EXPECT_EQ(0u, decoded->leavesQty_) << "[OrderCodec] round-trip preserves leavesQty";
        EXPECT_EQ(SELL_SIDE, decoded->side_) << "[OrderCodec] round-trip preserves side";
        EXPECT_DOUBLE_EQ(1.2650, decoded->price_) << "[OrderCodec] round-trip preserves price";
