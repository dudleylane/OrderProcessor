...
public:
    ...
    SubscriptionManager *dettach() override
    {
        return nullptr;
    }
...
{
    EXPECT_NO_THROW({ SubscriptionLayerImpl layer; });
}
...
TEST_F(SubscrManagerTest, ConcurrentAddSubscriptions)
{
    ...
    for (int t = 0; t < numThreads; ++t)
    {
        threads.emplace_back(
            [manager, t, subscriptionsPerThread, &successCount]()
            {
...
TEST_F(SubscrManagerTest, ConcurrentGetSubscribers)
{
    ...
    {
        threads.emplace_back(
            [manager, buyOrder, queriesPerThread, &matchCount]()
            {
