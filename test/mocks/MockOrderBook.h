...
public:
    MockOrderFunctor()
    {
        side_ = COP::INVALID_SIDE;
    }
    explicit MockOrderFunctor(COP::Side side)
    {
        side_ = side;
    }

    void setSide(COP::Side side)
    {
        side_ = side;
    }
...
public:
    ...
    SourceIdT instrument() const override
    {
        return instr_;
    }
    bool match(const IdT &, bool *) const override
    {
        return matchResult_;
    }

    void setInstrument(const SourceIdT &instr)
    {
        instr_ = instr;
    }
    void setMatchResult(bool result)
    {
        matchResult_ = result;
    }
    void setSide(COP::Side side)
